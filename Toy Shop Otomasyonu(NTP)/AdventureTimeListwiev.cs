...
{
    public partial class AdventureTimeListwiev : Form
    {
        public AdventureTimeListwiev()
        {
            InitializeComponent();
        }
        SQLbaglanti bgl = new SQLbaglanti();
        private void AdventureTimeListwiev_Load(object sender, EventArgs e)
        {
            button1.Show();
            goster();
        }
        void goster()
        {
            ...
            if (bgl.baglanti().State == ConnectionState.Closed) //Bağlantı kapalıysa aç
                bgl.baglanti().Open();
            SqlCommand komut = new SqlCommand("select * from Tbl_Satis WHERE Kategori='" + "Adventure Time" + "'", bgl.baglanti());
        ...
        private void tuna_Paint_1(object sender, PaintEventArgs e)
        {
        ...
        private void AdventureTimeListwiev_FormClosing(object sender, FormClosingEventArgs e)
        {
            string hatirla = "";
            if (bgl.baglanti().State == ConnectionState.Closed) //Bağlantı kapalıysa aç
                bgl.baglanti().Open();
